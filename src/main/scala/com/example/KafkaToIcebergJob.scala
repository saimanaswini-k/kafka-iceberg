...
object KafkaToIcebergJob {
  ...
  def main(args: Array[String]): Unit = {
    try {
      ...
      // Configure serialization to use Avro instead of Kryo
  ...
  private def createKafkaSource(): KafkaSource[String] = {
    ...
    KafkaSource.builder[String]()
      ...
      .setProperty("fetch.max.bytes", "5242880")  // 5MB max fetch size
      .setProperty("max.partition.fetch.bytes", "1048576")  // 1MB per partition
      .build()
  }
} 